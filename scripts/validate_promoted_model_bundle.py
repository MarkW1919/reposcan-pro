...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate a promoted model bundle config and its artifact manifests.")
...
def main() -> int:
    ...
    from reposcan_inference.promotion import validate_promoted_model_stack

    args = parse_args()
    model_stack = load_model_config(repo_root / args.model_config)
    report = validate_promoted_model_stack(model_stack)

    if args.as_json:
        print(json.dumps(report.model_dump(mode="json"), indent=2))
    else:
        print(f"Promoted bundle: {report.stack_name}")
        print(f"Runtime ready: {'yes' if report.runtime_ready else 'no'}")
        print(f"Promotion ready: {'yes' if report.ready else 'no'}")
        for stage in report.stages:
            print(f"- {stage.stage}: ready={'yes' if stage.ready else 'no'}")
            if stage.artifact_manifest_path:
                print(f"  manifest: {stage.artifact_manifest_path}")
